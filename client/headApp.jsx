...
class HeadApp extends React.Component {
  ...
  move() {
    ...
    if (document.getElementById("backdrop" + 1 * this.state.gamesToSee) && this.state.gamesToSee === 1) {
      // $("#backdrop" + 1 * this.state.gamesToSee).first().remove();
    }

    if (document.getElementById("backdrop" + 2 * this.state.gamesToSee)) {
      // $("#backdrop" + 2 * this.state.gamesToSee).previous().remove();
      console.log(document.getElementById("backdrop" + 2 * this.state.gamesToSee).length)
    }
    ...
    if (this.state.totalRenders === 1) {
      const svg = d3.select("#map" + 1 * this.state.gamesToSee).append("svg:svg")
    ...
    if (this.state.totalRenders === 2) {
      ...
      for (let z = 0; z < this.state.playerID2.length; z++) {
        const checking = this.state.playerID2[z][1];

        // INITIAL RENDERING OF POSITION AT FRAME 0 FOR SIMPLICITY
        svg2.append('svg:g').attr("id", "champIcon" + 2 * this.state.gamesToSee).selectAll("image")
          .data([this.state.pos2[z]])
          .enter().append("svg:image")
            .attr('xlink:href', 'http://ddragon.leagueoflegends.com/cdn/' + this.state.patch2 + '/img/champion/' + this.state.champImg2[checking] + '.png')
            .attr('x', d => { return xScale(d[0]) })
            .attr('y', d => { return yScale(d[1]) })
            .attr('class', 'image')
            .style({ 'width': '17px', 'height': '17px' });         
  ...
  addStatChoice(count) {
    ...
    if (this.state.totalRenders === 2) {
      const w = 550, 
          h = 300,
          svg2 = d3.select("#chart" + count * this.state.gamesToSee)
                  .append("svg:svg")
                  .attr("width", w)
                  .attr("height", h)
                  .attr("id", "allStat" + count);
  ...
  addItemVisuals(count) {
    ...
    if (this.state.totalRenders === 2) {
      const w = 304,
          h = 400,
          svg2 = d3.select("#builds" + count * this.state.gamesToSee)
                  .append("svg:svg")
                  .attr("width", w)
                  .attr("height", h)
                  .attr("id", "allItems" + count);
    ...
    if (this.state.secondToggle === true && this.state.toggle === true) {
      return (
        <div className="resultingInfo">
          ...
          <TimeStamp gamesToSee={this.state.gamesToSee} totalRenders={this.state.totalRenders} timeline1={this.state.allowScroll1} conversion={this.state.num1} timeline1={this.state.allowScroll1} />
